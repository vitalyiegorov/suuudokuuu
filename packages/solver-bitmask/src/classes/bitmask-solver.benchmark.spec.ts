...
const measureChecksPerSecond = (solver: SolverInterface, grid: Uint8Array, iterations: number): number => {
    ...
    return Math.round((iterations / Math.max(MINIMUM_ELAPSED_MILLISECONDS, Date.now() - start)) * MILLISECONDS_PER_SECOND);
};
...
describe('BitmaskSolver benchmark', () => {
    it('measures countSolutions throughput for both engines on the Royle 17-given puzzle', () => {
        ...
        // eslint-disable-next-line no-console -- benchmark spec reports throughput numbers for human review, not application runtime output
        console.info(`[benchmark] Royle 17-given puzzle: DLX ${dlxCountPerSecond} checks/s, Bitmask ${bitmaskCountPerSecond} checks/s`);
    ...
    it('measures countSolutions throughput for both engines on a fixed 22-given grid', () => {
        ...
        // eslint-disable-next-line no-console -- benchmark spec reports throughput numbers for human review, not application runtime output
        console.info(`[benchmark] Fixed 22-given grid: DLX ${dlxCountPerSecond} checks/s, Bitmask ${bitmaskCountPerSecond} checks/s`);
