...
describe('NakedPairTechnique', () => {
    ...
    it('should find all NakedPair opportunities in field', () => {
        const field = createFieldFromString(
            '3456789..' +
                '.......12' +
                '.......34' +
        ...
        const nakedPairResults = results.filter((result) => result.technique === SolutionTechniqueEnum.NakedPair);

        expect(nakedPairResults.length).toBeGreaterThanOrEqual(0);
    });
});
