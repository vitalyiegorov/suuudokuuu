...
export class JellyfishTechnique extends BaseTechnique {
    ...
    canApply(cell: CellInterface, candidates: number[]): boolean {
        ...
        for (const value of candidates) {
            if (this.hasJellyfishForValue(value)) {
                return true;
            }
        }
    ...
    private hasJellyfishForValue(value: number): boolean {
        const rowsWithValue = this.findRowsWithCandidates(value, 2, 4);
        const colsWithValue = this.findColsWithCandidates(value, 2, 4);
