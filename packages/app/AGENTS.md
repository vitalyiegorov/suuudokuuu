...
8. The hint surface has one fixed height per viewport and stepping never changes it. `useHintSurfaceMetrics` measures the tools slot, `gameGetHintSurfaceMetrics` adds the deterministic numpad height from `gameGetNumpadHeight` and clamps the result between `HintSurfaceNarrowMinHeightConstant` and `HintSurfaceNarrowHeightConstant`, and `HintPanel` renders at exactly that height with the narration capped to the matching `numberOfLines`. Never let narration length drive the surface height. On compact layouts the surface is bottom anchored over the tools row and the numpad, both of which stay mounted (the tools row only fades to `opacity: 0` with `pointerEvents="none"`), so the board keeps its position and size when a hint opens, advances or closes. Wide layouts keep the bounded bottom-center card at `HintSurfaceWideHeightConstant`.
