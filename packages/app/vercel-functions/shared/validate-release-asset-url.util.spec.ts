...
const TagName = 'development-123';
const ValidUrl = 'https://github.com/vitalyiegorov/suuudokuuu/releases/download/development-123/suuudokuuu-development.ipa';
...
describe('validateReleaseAssetUrl', () => {
    ...
    it.each([
        ...
        ValidUrl.replace(TagName, 'development-122'),
        ValidUrl.replace(DevelopmentIpaAssetName, 'other.ipa'),
        `${ValidUrl}?download=1`,
        `${ValidUrl}#asset`,
        ValidUrl.replace(TagName, `${TagName}/../development-122`),
        ValidUrl.replace(TagName, 'development-123%2F..%2Fdevelopment-122')
    ...
    it('rejects traversal supplied as an expected tag or asset name', () => {
        const traversalTag = 'development-123/../development-122';
