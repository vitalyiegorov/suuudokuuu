...
│   └── utils/                   # givens, varint, aggregate-trailer, handoff-extras, timeline-event-stream codecs
...
8. Be deliberate about failure behavior: structural decode errors (bad version, unknown payload kind, unknown timeline event subcode, out-of-range positions, invalid base64url) throw. The aggregate trailer is the one exception — it silently degrades to `{ pencilCount: null, screenshotCount: null }` on missing, truncated, or version-mismatched trailer bits instead of throwing. Preserve that split unless callers and tests change together.
...
- `SharedPayloadKindEnum`: `Puzzle = 0` writes only the header and field — no timeline events, handoff extras, or aggregate trailer, and always decodes with `elapsedTime: 0` and a null aggregate trailer. `Handoff = 1` and `Challenge = 2` both write timeline events and, when both counts are known, the aggregate trailer; only `Handoff` additionally writes handoff extras between the two.
...
- Aggregate trailer (`aggregate-trailer-codec.util.ts`, `Handoff`/`Challenge` kinds, written only when both `pencilCount` and `screenshotCount` are non-null): 4-bit trailer version, then `pencilCount` and `screenshotCount` as 8-bit/16-bit varints. Missing, truncated, or version-mismatched trailer bits decode as `{ pencilCount: null, screenshotCount: null }` instead of throwing, so `null` stays distinguishable from a genuine `0` and older/foreign payloads still decode.
- Rating trailer (`rating-codec.util.ts`, all three payload kinds, written unconditionally as the final field): 4-bit trailer version, a 7-bit `rating` (rating × 10; `0` means unknown), a 1-bit `isRatingCeiling` flag, then a 3-bit `difficulty` (`0` means unknown/infer-from-blanks; `1`+ are ordinal `DifficultyEnum` positions from the `generator` package, kept as a plain number here since this package has no dependency on `generator`). Missing, truncated, or version-mismatched trailer bits decode as `{ rating: 0, isRatingCeiling: false, difficulty: 0 }` instead of throwing, mirroring the aggregate trailer's degrade-to-unknown behavior. Unlike the aggregate trailer, the `Puzzle` kind carries this trailer too.
